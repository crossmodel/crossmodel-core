...
export function Header({ name, id, iconClass }: HeaderProps): React.ReactElement {
   ...
   const startContent = (
      <div className='form-header-content'>
         {iconClass && <i className={`codicon ${iconClass} form-header-icon`} />}
         <h5 className='form-title'>
            {name}
            {saveModel && dirty ? '*' : ''}
         </h5>
         {!untitled && <span className='form-id'>ID: {id}</span>}
      </div>
   );
   ...
   return (
      <div className='form-header'>
         ...
         <Toolbar start={startContent} end={endContent} className='form-toolbar' />
      </div>
   );
}
