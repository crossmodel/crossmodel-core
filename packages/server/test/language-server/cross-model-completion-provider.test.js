...
describe.only('CrossModelCompletionProvider', () => {
    ...
    `;
    beforeAll(async () => {
        await parseDocuments({ services, text: dataModelA, documentUri: testUri('projectA', 'datamodel.cm') }, { services, text: address, documentUri: testUri('projectA', 'address.entity.cm') }, { services, text: order, documentUri: testUri('projectA', 'order.entity.cm') });
        await parseDocuments({ services, text: dataModelB, documentUri: testUri('projectB', 'datamodel.cm') }, { services, text: customer, documentUri: testUri('projectB', 'customer.entity.cm') });
    });
...
//# sourceMappingURL=cross-model-completion-provider.test.js.map