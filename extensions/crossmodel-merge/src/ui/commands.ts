...
import { findFiles, getMergeBase, getRelativePath, getRepository, readFileAtRef } from '../io/git-io.js';
import { getCrossModelServices, parseFile, parseText } from '../io/parse.js';
...
export function registerCommands(
   ...
   treeProvider: MergeTreeDataProvider
): void {
            ...
            for (const fileUri of files) {
               try {
                  ...
                  try {
                     ...
                     base = await parseText(baseText, fileUri);
                  } catch {
      ...
      await vscode.window.withProgress(
         ...
         async () => {
            ...
            for (const fileUri of files) {
               try {
                  ...
                  try {
                     ...
                     theirs = await parseText(theirsText, fileUri);
                  } catch {
                     theirs = undefined;
                  }
            ...
            vscode.window.showInformationMessage(
               `Found ${changes.length} changed file(s). Conflicts are marked with ⚠️`
            );
         }
      );
   } catch (error) {
         ...
         async () => {
            ...
            for (const change of changes) {
               ...
               try {
                  ...
                  // Apply the change
                  const modified = applySelected(
                     ours,
                     change,
                     c => treeProvider.isSelected(c.id),
                     reflection,
                     HINTS
                  );
            ...
            if (success) {
               vscode.window.showInformationMessage(
                  `Applied changes to ${fileChanges.size} file(s)`
               );
               // Refresh the tree
