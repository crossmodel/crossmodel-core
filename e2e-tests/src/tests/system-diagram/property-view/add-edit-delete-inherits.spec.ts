...
test.describe('Add/Edit/Delete inherits via properties view', () => {
   ...
   test('Add multiple parents via properties view', async () => {
      ...
      expect(await entityEditor.textContentOfLineByLineNumber(2)).toMatch('id: AddEditDeleteInherits');
   ...
   test('Modify existing parent entry via properties view', async () => {
      ...
      // Re-open and modify
      const diagramEditor2 = await app.openCompositeEditor(SYSTEM_DIAGRAM_PATH, 'System Diagram');
      const propertyView2 = await diagramEditor2.selectLogicalEntityAndOpenProperties(EMPTY_ENTITY_ID);
      const form2 = await propertyView2.form();

      const inherit = await form2.inheritsSection.getInherit('Customer');
      await inherit.setParentId('Order');
      await inherit.save();
      await propertyView2.save();
      await diagramEditor2.saveAndClose();
   ...
   test('Remove parent via properties view', async () => {
      ...
      const existing = await form2.inheritsSection.findInherit('Customer');
